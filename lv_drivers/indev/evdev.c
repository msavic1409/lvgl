...
#if NEW_SCREEN
#define GET_ABS_VALUE(val) (4096 - (val))
#else
#define GET_ABS_VALUE(val) (val)
#endif
...
bool evdev_read(lv_indev_data_t * data)
{
    ...
    while(read(evdev_fd, &in, sizeof(struct input_event)) > 0) {
        if (in.type == EV_REL) {
            ...
            if (in.code == ABS_X)
                tmp_x = GET_ABS_VALUE(in.value);
            else if (in.code == ABS_Y)
                tmp_y = GET_ABS_VALUE(in.value);
            ...
            if (in.code == SYN_REPORT) {
                evdev_root_x = (tmp_x * 1024) / 4096;
                evdev_root_y = (tmp_y * 600) / 4096;
            }
        }
    }
